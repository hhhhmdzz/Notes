...
|          Layer Norm          | $bsh \to bsh$ |   $X$   |         $bsh + 2bs \approx bsh$          | 输入 |
...
| $S = \text{softmax}\left( \frac{S}{\sqrt{h}} \right)$ |      $bs^2 \to bs^2$      | $\approx 4bs^2a$ |         |
...
| :---------------: | :------------: | :-------------: | :-----------------------: | :--------: |
|    Layer Norm     |                |  $bsh \to bsh$  |      $\approx 6bsh$       | 均值、方差 |
...
这是前向传播的总计算量，在“**前向传播和反向传播之间的关系**”中已经推导反向传播计算量为前向传播计算量的两倍，所以一个批量的数据的总计算量为：
